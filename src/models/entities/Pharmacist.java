    ...
    public class Pharmacist extends Staff {
        ...
        public void updatePrescriptionStatus(String appointmentId, String medicineName, String newStatus) {
            String filePath = FilePaths.APPTOUTCOME.getPath();
            boolean appointmentFound = false;
            ...
            try (FileInputStream fis = new FileInputStream(filePath);
                ...
                for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                    ...
                    if (apptIdCell != null && apptIdCell.getStringCellValue().equals(appointmentId)) {
                        ...
                        List<String> quantities = Arrays.asList(quantityCell.getStringCellValue().split(",\\s*"));
                        ...
                        for (int j = 0; j < medicines.size(); j++) {
                            ...
                            if (currentMedicine.equalsIgnoreCase(medicineName)) {
                                if (!currentStatus.equalsIgnoreCase("DISPENSED")) {
                                    statuses.set(j, newStatus); // Update status
                                    dispensedQuantity = Integer.parseInt(quantities.get(j));
                                    updateMade = true;
                                    medicineFound = true;
                                    System.out.println("Prescription status updated to " + newStatus +
                                            " for Appointment ID: " + appointmentId + ", Medicine: " + medicineName);
                                } else {
        ...
        public boolean hasPendingMedicines(String appointmentId) {
            ...
            try (FileInputStream fis = new FileInputStream(filePath);
                ...
                for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                    ...
                    if (apptIdCell != null && apptIdCell.getStringCellValue().equals(appointmentId)) {
                        // Check for any status other than "DISPENSED"
                        if (statusCell != null && !statusCell.getStringCellValue().equalsIgnoreCase("DISPENSED")) {
                            hasPending = true;
                            break;
                        }
                    }
